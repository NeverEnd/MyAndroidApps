...
public class WidgetDataBaseAdapter implements DataBaseAdapter
{
	...
	public void insert(int widgetId, int x, int y, int spanX, int spanY, int screen, WidgetBean.WidgetEnum type,int scene)
	{
		ContentValues cv = new ContentValues();
		...
		cv.put(WidgetBean.DateBase.COLUMN_SCENE, scene);
		long id = _SQLiteDatabase.insert(WidgetBean.DateBase.TABLENAME, null, cv);
		System.out.println("insert_success->id"+id);
		System.out.println("insert_success->widegetId"+widgetId);
	}
