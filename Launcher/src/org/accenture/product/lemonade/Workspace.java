...
public class Workspace extends ViewGroup implements DropTarget, DragSource, DragScroller
{
	...
	void swithScreen(int currentScreen)
	{
		if (!mScroller.isFinished())
			mScroller.abortAnimation();
		clearVacantCache();
	...
	void hideCellLayout(int currentScreen)
	{
		CellLayout cell = ((CellLayout) getChildAt(currentScreen));
		for (int i = 0; i < cell.getChildCount(); i++)
		{
			cell.getChildAt(i).setVisibility(View.INVISIBLE);
		}
	}

	void showCellLayout(int currentScreen)
	{
		CellLayout cell = ((CellLayout) getChildAt(currentScreen));
		for (int i = 0; i < cell.getChildCount(); i++)
		{
			cell.getChildAt(i).setVisibility(View.VISIBLE);
		}
	}
	...
	private void updateWallpaperOffset(int scrollRange)
	{
		...
		if (token != null)
		{
			...
			mWallpaperManager.setWallpaperOffsets(getWindowToken(), Math.max(0.f, Math.min(getScrollX() / (float) scrollRange,
					1.f)), 0);
		}
	}
			...
			case MotionEvent.ACTION_UP:
				if (mTouchState == TOUCH_STATE_SCROLLING)
				{
					...
					if (velocityX > SNAP_VELOCITY
							&& (mCurrentScreen > (Preferences.getInstance().getEndlessScrolling() ? -1 : 0)))
					{
						...
						snapToScreen(Math.min(whichScreen, bound), velocityX, true);
							...
							&& mCurrentScreen < getChildCount() - (Preferences.getInstance().getEndlessScrolling() ? 0 : 1))
					{
	...
	private void snapToScreen(int whichScreen, int velocity, boolean settle)
	{
		...
		whichScreen = Math.max((Preferences.getInstance().getEndlessScrolling() ? -1 : 0), Math.min(whichScreen,
				getChildCount() - (Preferences.getInstance().getEndlessScrolling() ? 0 : 1)));
		clearVacantCache();
	...
	public void onDrop(DragSource source, int x, int y, int xOffset, int yOffset, DragView dragView, Object dragInfo)
	{
		...
		else
		{
			...
			if (mDragInfo != null)
			{
				...
				mTargetCell = estimateDropCell(x - xOffset, y - yOffset, mDragInfo.spanX, mDragInfo.spanY, cell, cellLayout,
						mTargetCell);
	...
	private void onDropExternal(int x, int y, Object dragInfo, CellLayout cellLayout, boolean insertAtFirst)
	{
		...
		switch (info.itemType)
		{
			case LauncherSettings.Favorites.ITEM_TYPE_SHORTCUT:
				if (info.container == NO_ID && info instanceof ShortcutInfo)
				{
					...
					List<ShortcutInfo> apps = mLauncher.getAppDB().getApps(new long[]
					{ newShortcutInfo.id });
					if (apps.size() == 1)
					{
						...
						try
						{
							...
							if (path != null)
							{
								...
								// picDrawable.setBounds(0,0,AllApps3D.ICON_WIDTH,AllApps3D.ICON_HEIGHT);
		...
		mLauncher.getModel().addOrMoveItemInDatabase(mLauncher, info, LauncherSettings.Favorites.CONTAINER_DESKTOP,
				mCurrentScreen, lp.cellX, lp.cellY);
	...
	public boolean acceptDrop(DragSource source, int x, int y, int xOffset, int yOffset, DragView dragView, Object dragInfo)
	{
		try
		{
			...
			boolean space = mVacantCache.findCellForSpan(mTempEstimate, spanX, spanY, false);
			if (false == space)
				Toast.makeText(mLauncher, mLauncher.getString(R.string.out_of_space), Toast.LENGTH_SHORT).show();
					...
					if (Intent.ACTION_MAIN.equals(intent.getAction()) && name != null)
					{
						...
						for (int k = 0; k < appCount; k++)
						{
							...
							if (name.equals(cname))
							{
								...
								((TextView) view).setCompoundDrawablesWithIntrinsicBounds(null, new FastBitmapDrawable(info
										.getIcon(mIconCache)), null, null);
							}
						}
					}
				}
